%% % @format
-module(murmurhash3_x86_32_timeslice).
...
-spec hash(Seed, Key) -> Out when Seed :: seed(), Key :: iolist(), Out :: murmurhash3_x86_32_timeslice_nif:out().
hash(Seed, Key) when ?is_seed(Seed) andalso ?is_key(Key) ->
    murmurhash3_x86_32_timeslice_nif:hash(Seed, Key);
